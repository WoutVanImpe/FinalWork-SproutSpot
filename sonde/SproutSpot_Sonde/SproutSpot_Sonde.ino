...
#include <ESPmDNS.h>
#include <time.h>                  // Voor tijd-functies
#include "SHT31.h"
#include <Digital_Light_TSL2561.h>

// --- GLOBALE OBJECTEN & CONFIG ---
...
// --- KALIBRATIE GRENZEN & PINNEN ---
const int SOIL_AIR_VALUE = 2765;   // Maximaal droog
const int SOIL_WATER_VALUE = 1690; // Maximaal nat
const int SOIL_PIN = A0;
const int BATTERY_PIN = A1;        // Spanningsdeler (100k + 33k). Doet nu ook USB detectie!
const int RESET_BUTTON_PIN = D2;    // D2 (GPIO 4) voor de fysieke knop (RX conflict opgelost)

// --- MODE SELECTIE ---
// Zet DEV_MODE aan voor korte wachttijden tijdens testen, uit voor productie
#define DEV_MODE

#if defined(DEV_MODE)
  const unsigned long CHARGE_UPLOAD_INTERVAL = 30000;     // 30 seconden
  const uint64_t SLEEP_TIME_SECONDS = 30;                  // 30 seconden
  const uint64_t INACTIVE_SLEEP_SECONDS = 30;              // 30 seconden
#else
  const unsigned long CHARGE_UPLOAD_INTERVAL = 180000;     // 3 minuten
  const uint64_t SLEEP_TIME_SECONDS = 900;                 // 15 minuten
  const uint64_t INACTIVE_SLEEP_SECONDS = 3600;            // 1 uur
#endif

// --- BATCHING & RETAIN LOGICA (RTC MEMORY) ---
...
// Timers voor de actieve Laad-modus (Conditie 4) zonder Deep Sleep
...
void setup() {
  ...
  // --- DRINGENDE FIX: Wacht tot de computer de USB-poort heeft hersteld ---
  // Dit zorgt ervoor dat je na een deep sleep wakeup de tekst niet mist!
  int timeout = 0;
  ...
  // --- EXTRA TEST PRINT: Laat direct zien WAAROM de chip is opgestart ---
...
void loop() {
  ...
  if (isPluggedIn() && WiFi.getMode() == WIFI_AP_STA) {
  ...
  if (isPluggedIn() && WiFi.status() == WL_CONNECTED) {
    ...
    delay(100); // Zeer snelle loop zodat de live-knop in de loop ook alert blijft
    return; 
  }

  // --- CONDITIE 2: METINGEN IN DE POT VIA INTERNE RTC KLOK ---
