i do# SproutSpot: Development History & Decisions
...
## 29/05/2026: Snoozed Notification Scheduler, Count Endpoint & Battery Warning

### Test Push Endpoint
- Added `POST /api/notifications/test-push` for manual push debugging without waiting for telemetry triggers

### Scheduler for Snoozed Notifications
- `notification.repository.ts` — `getDueSnoozedNotifications()`, `activateSnoozedNotification()`, `rescheduleSnoozedNotification()`, `getUnacknowledgedCount()`
- `scheduler.service.ts` — calls `processSnoozedNotifications()` on every tick
- `telemetry.service.ts` — `processSnoozedNotifications()` delivers within notification window or reschedules outside it

### Notification Count Endpoint
- `GET /api/notifications/count` — returns `{ count }` for unacknowledged notifications
- Used by NavHeader red badge on home screen

### Battery Warning at 3.5V
- `checkBattery()` now triggers `BATTERY_WARNING` at 3.5V (~20%) in addition to `BATTERY_LOW` at 3.12V (~10%)
- Auto-resolves when voltage recovers above threshold
- Gives user earlier notice before critical low state

### Reset Uses 12h Snooze
- `resetNotificationState()` now calls `snoozeForHalfDay()` instead of hardcoded 12h

### Key Decisions
1. Separate `BATTERY_WARNING` and `BATTERY_LOW` thresholds give users two chances to charge before probe dies
2. Snoozed notifications outside window are rescheduled (not delivered immediately) — respects user's quiet hours
3. Test-push endpoint allows frontend devs to verify push delivery without setting up telemetry

### Files Modified
- `backend/controllers/notification.controller.ts` — test-push + count handlers
- `backend/repositories/notification.repository.ts` — 5 new snooze/count methods
- `backend/routes/notification.routes.ts` — count route
- `backend/services/notification.service.ts` — `getUnacknowledgedCount()`, snooze refactor
- `backend/services/scheduler.service.ts` — wired `processSnoozedNotifications()`
- `backend/services/telemetry.service.ts` — `processSnoozedNotifications()`, `BATTERY_WARNING`

### TypeScript
- Backend: zero errors

## 30/05/2026 (Morning): Dynamic Plant Stages, Real Sensor Data & Badge Fixes

### Notification Count Response Path
- NavHeader red badge was reading from wrong response property — fixed to match actual API shape

### Notification Badge Restyling
- White count badge positioned inside AccountMain button with absolute positioning
- Button width set to match other menu buttons (was narrower, looked inconsistent)
- Full visual restyle: badge size, font, colors

### Graph Modal NaN Fix
- `aggregateData()` returned empty array when readings had no time range overlap
- Added `points.length > 0` guard before rendering SVG circles

### Dynamic Stages from Backend
- `plantEnricher.ts` now includes `stages` (full stage definitions with thresholds/labels) and `totalDays` in enriched response
- Frontend `GardenPlant` type updated with `stages` and `totalDays` fields
- `plant-detail.tsx` rewired to use dynamic stages from API instead of hardcoded arrays

### Real Sensor Status on Plant Detail
- TechnicalOverview now shows real probe data (battery %, soil moisture, light, temperature) from latest telemetry
- Previously showed mock "65% / 450 / 24°C" values

### Sent-Only Notification Count
- Badge counts only `sent` notifications (not snoozed) — users shouldn't be nagged about snoozed alerts

### Snooze Reduced to 6h
- Changed from 12h to 6h for faster re-delivery of unresolved issues

### Stage Fallback Guard
- `buildStages()` now guards against `undefined` stages from backend — returns empty array instead of crashing

### Restored Hardcoded Stage Descriptions
- When dynamic data is missing, plant-specific hardcoded stage descriptions serve as fallback
- Ensures plant detail never shows blank stage section

### Files Modified
- `backend/utils/plantEnricher.ts` — added `stages`, `totalDays`
- `backend/repositories/notification.repository.ts` — sent-only count, 6h snooze
- `frontend/app/(garden)/plant-detail.tsx` — dynamic stages, fallback guard, real sensor data
- `frontend/components/pages/garden/plantDetail/RequirementsSection.tsx` — real thresholds
- `frontend/components/pages/garden/plantDetail/TechnicalOverview.tsx` — real probe data
- `frontend/components/pages/garden/plantDetail/GraphModal.tsx` — NaN guard
- `frontend/components/pages/account/main/AccountMain.tsx` — badge position + button width
- `frontend/components/shared/header/NavHeader.tsx` — count response path
- `frontend/services/garden.ts` — `stages`, `totalDays` types

## 30/05/2026 (Mid-day): Graph Modal Date Labels, Nav Position Experiments & Reverts

### Graph Modal Date Labels
- `formatLabel()` now returns day/month (e.g. "30/05") for aggregated weekly/3-weekly views
- Time-only format (e.g. "14:00") preserved for 24h view
- Prevents label width overflow on dense charts

### Scroll Indicator Hidden
- `StyledView` added `showsVerticalScrollIndicator={false}` — cleaner look, no distracting scroll bar on page load

### Nav Header Position Fixes (3 Attempts, All Reverted)

**Attempt 1: Padding in Fixed Containers** (`36a3440`)
- Moved `paddingTop` from NavHeader to fixed containers so content stays below the transparent header
- Required changes to explore, garden, and account page layouts

**Attempt 2: StyledView Wrapper** (`f9e75a4`)
- Wrapped entire StyledView in a fixed outer `<View>` with padding instead
- Avoids per-page changes but adds wrapper nesting

**Attempt 3: Reverted** (`99d8a09`, `2f16c66`)
- Both approaches caused layout regressions on pages with scroll content
- Reverted to original pattern: NavHeader sits above StyledView, pages manage their own top padding

### BlurView Frosted-Glass Effect (Attempted then Reverted)
- `44cb344`: Wrapped NavHeader with `expo-blur` BlurView for frosted-glass transparency effect — NavHeader shows page content blurred behind it
- `fdc9f91`: Reverted — performance concerns on lower-end Android devices + visual inconsistency across Android versions (some don't support frosted glass)

### Files Modified
- `frontend/components/pages/garden/plantDetail/GraphModal.tsx` — date labels
- `frontend/components/style/StyledView.tsx` — scroll indicator
- `frontend/components/shared/header/NavHeader.tsx` — BlurView experiment + revert
- `frontend/app/(explore)/explore.tsx` — padding changes (reverted)
- `frontend/app/(garden)/garden.tsx` — padding changes (reverted)
- `frontend/app/(account)/account.tsx` — padding changes (reverted)

## 30/05/2026 (Afternoon): Dependency Hell — React Version Mismatch

### Problem
React Native 0.81.5 bundles `react-native-renderer@19.1.0` but npm resolved `react@19.2.6`, causing "React 19.2.6 vs 19.1.0" mismatch warnings and potential runtime instability.

### Attempted Fixes (5 Commits + 3 Reverts)

| Commit | Action | Result |
|---|---|---|
| `c9092e5` | Downgraded react to 19.1.0, expo-clipboard to ~8.0.8, expo-router bump | Seemed to work |
| `725e353` | Added react-dom@19.1.0 override | Still had mismatch |
| `00a72c1` | Reverted `725e353` | Regression |
| `e941131` | Reverted `c9092e5` | Back to broken state |
| `f9b552d` | Final: react@19.1.0 + overrides for react-dom + react | ✅ Stable |

### Final Fix (`f9b552d`)
- `react`: `19.2.6` → `19.1.0`
- `expo`: `~54.0.33` → `~54.0.35` (minor bump)
- `expo-router`: `~6.0.23` → `~6.0.24` (minor bump)
- `react-native-svg`: `^15.12.1` → `15.12.1` (pin exact)
- `expo-clipboard`: `^55.0.13` → `~8.0.8` (SDK 54 compatible)
- Added `overrides` block: `react-dom@19.1.0`, `react@19.1.0`
- Added `react-dom` as direct dependency at `19.1.0`
- Verified: `npm ls react` → `19.1.0` across all packages

### Struggle
- Took 5 commits and 3 reverts over ~50 minutes (11:48 → 12:37)
- Reverts caused confusion because `package-lock.json` had conflicting resolutions
- `npm install --legacy-peer-deps` was required every time due to incompatible peer dep ranges
- Final fix needed both pinning AND overrides — neither alone was sufficient
...
## 30/05/2026 (Late Afternoon): Bug Fix Round — Stage Bar, Graphs, Auto-Refresh

### StatusHeader Arrows
- Left/right chevron arrows hidden when `items.length <= 1` — no point swiping a single plant
- `542ac02`

### Stage Bar 1-Based → 0-Based
- Backend stores `current_stage_order` starting at 1, but frontend `buildStages()` produces a 0-indexed array
- `currentStageIndex` now subtracts 1: `(plant.stage.current || 1) - 1`
- `f00b9af`

### Auto-Refresh on Screen Focus (`f3356dc`)
- Replaced `useEffect([], [])` with `useFocusEffect(useCallback(...))` in `garden.tsx`, `account.tsx`, `plant-detail.tsx`
- Garden page shows loading spinner during refresh
- Account page moves data fetching into the focus callback (was in `useEffect`)
- Ensures fresh data every time user navigates to the screen

### Graph Modal Label Overlap Fix (3 Commits)

**1. Dense Data Aggregation** (`40f7836`):
- `aggregateData()` now uses 30-min buckets for 24h views with >96 readings (instead of returning raw data)
- Threshold: `MAX_RAW = 96` (one reading per 15 min max for 24h)
- Buckets: 30-min for 24h, 1h for 1w, 24h for 3w

**2. Label Spacing** (`8835f7e`):
- `getDataLabel()` step calculation changed from `floor(total/5)` to `ceil((total-1)/4)`
- Produces exactly 5 evenly spaced labels regardless of data count

**3. Light Unit Alignment** (`a3c8cf4`):
- `buildMetrics()` now converts raw lux to percentage using `lux / 50000 * 100`
- Backend thresholds (`optimalMin=20`, `optimalMax=160`) are already percentages
- Fixes invisible optimal zone on light graph (data was 0-50000, zone was 20-160)

### PlantSheet: "Alles is in orde"
- When `advice` array is empty, shows "Alles is in orde, er is geen actie nodig." in gray
- Previously showed nothing — user couldn't tell if loading or everything was fine
- `970a3c9`

### Files Modified
- `frontend/components/pages/home/statusHeader/StatusHeader.tsx` — arrow guard
- `frontend/app/(garden)/plant-detail.tsx` — stage index fix, useFocusEffect
- `frontend/app/(garden)/garden.tsx` — useFocusEffect
- `frontend/app/(account)/account.tsx` — useFocusEffect
- `frontend/components/pages/garden/plantDetail/GraphModal.tsx` — aggregation, labels, light unit
- `frontend/components/pages/garden/plantSheet/PlantSheet.tsx` — empty advice fallback

### TypeScript
- Frontend: zero errors

## 31/05/2026: Stage Validation Flow, Telemetry Handling & Plant Detail Refresh

### Growth Stage Validation (End-to-End)
- **Backend scheduler** (`checkStageAdvancement()` in telemetry.service.ts): finds plants where `current_stage_start_date + stage_duration_days < NOW()`
- Creates `stage_validation` notifications via existing `dispatchNotificationForIssue()` path
- Deduplication: only creates if no existing `stage_validation` notification with `created_at > last_stage_update`
- **Frontend milestone cards**: "Herinner mij" + "Valideer" buttons on `stage_validation` notifications
- **Validation flow**: "Valideer" → `ValidateStep1` (how to recognize the new stage) → `ValidateStep2` (what changes) → calls `advanceStage()` API + acknowledges notification → returns to main
- **Snooze**: "Nog niet" button snoozes for 48h via `hours=48` body param on the reset endpoint (default is 6h)
- Plant name passed through `NotificationItem` to validation screens (no extra API call)

### Missing Telemetry Handling (`7d890bf`)
- `hasTelemetry` boolean added to enriched plant response (`plantEnricher.ts`)
- Plant-detail: when `!hasTelemetry`, status bars (water/light/temp) are hidden, shows "Nog geen metingen ontvangen" message
- PlantSheet: conditionally suppresses advice section when no data exists
- Prevents false "Alles is in orde" for a plant with no probe readings

### Navbar Account Page Fix
- `CustomTabBar`: `visibleIndex()` returns `-1` for `"(account)"` segment (it's not in the tab config)
- Removed `setValue` snap call on tab press — prevents home tab being highlighted when on account page

### Blank Screen Guard (`8dc320d`)
- `plant-detail.tsx:132`: Replaced `return null` with a safe `<View>` showing back button + "Plant niet gevonden."
- Prevents white screen crash when plant data hasn't loaded yet

### Preload Readings on Focus
- Moved `getReadings()` fetch out of `graphVisible` guard — runs on every focus regardless
- Eliminates empty-chart flash when opening graph modal for the first time

### Graph Loading State
- `GraphModal.tsx`: shows "Gegevens laden..." during fetch, "Geen meetgegevens beschikbaar." when readings array is empty
- New `readingsLoading` boolean prop from parent

### Validation Flow Preserved on Tab Switch
- `account.tsx`: `isValidatingRef` ref prevents `useFocusEffect` from resetting `currentView` when user is mid-validation
- Tapping another tab and returning keeps them on the validation step

### Alert on Dismiss/Snooze Error
- `account.tsx:109,118`: `Alert.alert` shown on API failure instead of silent `console.error`
- User gets feedback when dismiss or snooze fails

### GET /api/gardens/:id Endpoint (`a994766`)
- New controller/service method (`garden.controller.ts:36-69`, `garden.service.ts:68-131`)
- Fetches single active user plant by ID, enriches with stage info, thresholds, latest telemetry
- Plant-detail calls `getPlantById()` on every focus via separate `useFocusEffect`
- `useRef` used for plant ID in callback to prevent infinite re-render when plant state updates

### Files Modified
- `backend/services/telemetry.service.ts` — `checkStageAdvancement()`, stage validation dispatch
- `backend/repositories/userPlant.repository.ts` — `findPlantsReadyForStageAdvancement()`
- `backend/repositories/notification.repository.ts` — notification creation for stage validation
- `backend/controllers/notification.controller.ts` — return `userPlantId`, `nextStageOrder`
- `backend/services/notification.service.ts` — optional `hours` body param on reset
- `backend/repositories/telemetry.repository.ts` — helper for stage advancement query
- `backend/services/scheduler.service.ts` — call `checkStageAdvancement()`
- `backend/utils/plantEnricher.ts` — `hasTelemetry` flag
- `backend/controllers/garden.controller.ts` — `getUserPlantById()` handler
- `backend/services/garden.service.ts` — single plant enrichment
- `backend/routes/garden.routes.ts` — `GET /:id` route
- `frontend/app/(account)/account.tsx` — validation flow wiring, `isValidatingRef`, Alert on error
- `frontend/app/(garden)/plant-detail.tsx` — blank screen guard, preload readings, `getPlantById()` on focus
- `frontend/components/pages/garden/plantDetail/GraphModal.tsx` — loading/empty states
- `frontend/components/pages/garden/plantSheet/PlantSheet.tsx` — `hasTelemetry` conditional, advice guard
- `frontend/components/pages/account/notifications/NotificationsView.tsx` — validation buttons
- `frontend/components/pages/account/validate/ValidateStep1.tsx` — "Nog niet" snoozes 48h
- `frontend/components/pages/account/validate/ValidateStep2.tsx` — confirm advances stage
- `frontend/components/shared/navbar/CustomTabBar.tsx` — account tab highlight fix
- `frontend/services/garden.ts` — `getPlantById()`, advanceStage types
- `frontend/services/notifications.ts` — `userPlantId`, `nextStageOrder` fields

### TypeScript
- Frontend: zero errors

### Key Decisions
1. `hasTelemetry` flag used to suppress warnings/advice when no data exists — prevents false "Alles is in orde" for a plant with no probe readings
2. Stage validation dedup uses `created_at > last_stage_update` so old snoozed notifications from a previous stage don't block new ones
3. "Nog niet" on ValidateStep1 snoozes for 48h vs default 6h — passed via optional `hours` body param
4. Plant name passed through NotificationItem instead of separate API call
5. Readings fetched on plant-detail focus (not on graph modal open) to prevent empty-chart flash
6. `useRef` for plant ID in `useFocusEffect` avoids infinite re-render when plant state updates
7. Separate `GET /api/gardens/:id` endpoint for single plant refresh (instead of refetching whole garden)

## 01/06/2026: Notification Resend, Outdoor Plant Temperature Fixes

### Notification Resend Every 12h (`40f34e5`)
- Added `last_reminded_at` column to `pending_notifications` table
- `getPendingReminderNotifications()` — returns `sent` notifications older than 12h with no recent reminder (`last_reminded_at > 12h` or `NULL`)
- `processSentNotifications()` — sends push via Expo API and updates `last_reminded_at` timestamp
- Scheduler calls `processSentNotifications()` on every tick alongside existing checks
- Within-window check: skips reminders outside user's quiet hours

### Snoozed Notifications Auto-Acknowledged When Issue Resolved
- `processSnoozedNotifications()` now checks `issue_id` on each due snoozed notification
- If linked `active_issue` has `resolved_at` set, auto-acknowledges instead of re-sending push
- Prevents re-alerting users about issues that have already been resolved

### Outdoor Plants: TEMP_TOO_HIGH Skipped
- `telemetry.service.ts:178`: Instant temperature anomaly check now guarded by `if (plantingType !== "outdoor")`
- Temperature isn't actionable outdoors — users can't cool down an outdoor plant

### Outdoor Plants: TEMP_TOO_LOW Daily Check Skipped
- `telemetry.service.ts:369`: `checkDailyTemperatureIntegral()` skips outdoor plants entirely
- Outdoor plants are naturally adapted to ambient temperature

### 2°C Frost Guardrail Kept for All Plants
- Frost warning at <2°C remains active for ALL plants regardless of `plantingType`
- Frost is actionable (bring inside / cover with fleece) even for outdoor plants

### HAVING COUNT > 0 on Daily Summaries
- `getDailyLightSummary()` and `getDailyTemperatureSummary()` both require `COUNT(pe.id) > 0`
- Prevents false alerts when probe is offline (battery unplugged) — no data means no issue

### `last_reminded_at` Migration Cleanup
- Column added to original migration `20260504095304_innit_migration.ts` via alter
- Standalone migration `20260601000001_add_last_reminded_at.ts` deleted
- Keeps schema changes in a single file for cleaner history

### Key Decisions
1. `last_reminded_at` added to original migration file (no separate migration) for cleaner schema
2. `HAVING COUNT(pe.id) > 0` prevents false alerts when probe sends no data
3. Outdoor plants skip `TEMP_TOO_HIGH` instant alerts and `TEMP_TOO_LOW` daily average — temperature isn't actionable outdoors
4. Frost guardrail (<2°C) kept for all plants — actionable regardless of planting type
5. Snoozed notifications with resolved linked issues auto-acknowledge instead of re-sending

### Files Modified
- `backend/migrations/20260504095304_innit_migration.ts` — added `last_reminded_at` column
- `backend/services/scheduler.service.ts` — call `processSentNotifications()`
- `backend/services/telemetry.service.ts` — `processSentNotifications()`, outdoor temp guards, snoozed resolve check
- `backend/repositories/notification.repository.ts` — `getPendingReminderNotifications()`, `updateRemindedAt()`, `findIssueById()`
- `backend/repositories/telemetry.repository.ts` — `HAVING COUNT > 0`, `planting_type` in daily summary join

### Files Deleted
- `backend/migrations/20260601000001_add_last_reminded_at.ts`

### TypeScript
- Backend: zero errors

## 01/06/2026: Bug Fix Round — Battery Utility, Scheduler Safety, Auth, Type Consolidation

### Backend: Shared batteryPercentage Utility
- Extracted `batteryPercentage()` from `telemetry.service.ts` and `plantEnricher.ts` into a shared `backend/utils/battery.ts`
- Both services now import from the utility — eliminates duplicate formula (was 2 copies with identical MIN_VOLTAGE/MAX_VOLTAGE constants)
- `6eadbbe`

### Backend: Scheduler Error Isolation
- Wrapped all scheduler interval tasks in `safeRun()` — catches and logs individual task failures without crashing subsequent tasks
- Prevents one failed check (e.g., DB timeout on `checkStaleProbes`) from blocking all others (e.g., `processSnoozedNotifications`, `checkStageAdvancement`)
- `fd04abb`

### Backend: Battery Priority Fix
- Restructured `checkBattery()` so `BATTERY_LOW` takes priority over `BATTERY_WARNING` — nested WARNING inside the LOW else-branch
- Previously: both could fire independently, creating conflicting issues; auto-resolve for WARNING ran even when LOW was active
- `68a7d54`

### Backend: Pair/Unpair Ownership Checks
- `pairProbe` and `unpairProbe` controllers now use `AuthenticatedRequest`, extract `userId`
- `pairProbe`: verifies `probe.user_id === userId` before linking
- `unpairProbe`: looks up `userPlant` via `UserPlantRepository.findByIdWithDetails()`, verifies ownership before unlinking
- `a161cd9`

### Frontend: Auth Placeholder Fix
- Login/signup no longer set a hardcoded placeholder user object — now calls `getProfile()` after auth and sets the real user
- `syncPushToken()` called with the real `push_token` from the profile
- `53cfb70`

### Frontend: Step Back-Stack Navigation
- `plant-step2/3/4` back buttons changed from `router.push(\`...\?vegId=${vegId}\`)` to `router.back()` — respects the actual navigation stack instead of hardcoding the previous route
- `38848c4`

### Frontend: Type Consolidation & Dead Code Removal
- **Removed** unused `VegetableDetail` + `VEGETABLE_DETAILS` from `frontend/data/vegetables.ts` (220 lines) — data lives on backend now
- **Extracted** `enrichedToGardenPlant()` from `garden.tsx` and `plant-detail.tsx` into `frontend/services/garden.ts` — single shared mapping function
- **Renamed** `NotificationItem` → `NotificationViewItem` in `NotificationsView.tsx` to avoid naming conflict with API type in `frontend/services/notifications.ts`
- **Replaced** `JSON.stringify(plant)` URL param in `PlantSheet.tsx` with simple `plant.id` — `plant-detail.tsx` now fetches fresh data via ID instead of parsing a serialized blob
- **Removed** unused `getUserPlants()`, `getCurrentStage()` from garden service
- **Removed** unused `humidity_pct` from `ReadingRecord` type and `GraphModal.aggregateData()`
- `29853fc`

### Frontend: Minor Cleanups
- `_layout.tsx`: removed dead `prevVisibleIndex` ref and write statements
- `PlantSheet.tsx`: added `isVisible` to useEffect dependency array (was stale closure on `[internalVisible]`)
- `StatusHeader.tsx`: guarded modulo operations against empty `items` array; added `useEffect` on `[items]` to update `colorAnim` when items change; added `useEffect` to React import
- `GraphModal.tsx`: removed stale `humidity_pct` from `aggregateData()`
- `648a0d3`

### Commits (9 total)
```
7a5148d feat(backend): extract shared batteryPercentage utility
6eadbbe feat(backend): migrate telemetry and plantEnricher to shared batteryPercentage utility
fd04abb fix(backend): wrap scheduler tasks in safeRun, log enrichPlant errors
68a7d54 fix(backend): prevent BATTERY_WARNING from firing when BATTERY_LOW is active
a161cd9 fix(backend): add ownership check to probe pair/unpair endpoints
53cfb70 fix(frontend): replace hardcoded auth placeholder with real getProfile()
38848c4 fix(frontend): use router.back() for correct back-stack navigation
29853fc refactor(frontend): consolidate types, extract enrichedToGardenPlant, remove dead code
648a0d3 fix(frontend): stale closure dep, dead ref, empty-array guards, colorAnim reactivity
```

### Files Modified (19)
- `backend/utils/battery.ts` — new file
- `backend/services/telemetry.service.ts` — battery utility migration + priority fix
- `backend/utils/plantEnricher.ts` — battery utility migration, catch logging
- `backend/services/scheduler.service.ts` — safeRun wrapper
- `backend/controllers/probe.controller.ts` — AuthenticatedRequest, userId extraction
- `backend/services/probe.service.ts` — ownership checks, UserPlantRepository import
- `frontend/context/AuthContext.tsx` — real profile fetch after login/signup
- `frontend/app/(explore)/plant-step2.tsx` — router.back()
- `frontend/app/(explore)/plant-step3.tsx` — router.back()
- `frontend/app/(explore)/plant-step4.tsx` — router.back()
- `frontend/data/vegetables.ts` — removed VegetableDetail, VEGETABLE_DETAILS
- `frontend/app/(account)/account.tsx` — NotificationViewItem rename
- `frontend/components/pages/account/notifications/NotificationsView.tsx` — type rename, getImageSource
- `frontend/app/(garden)/garden.tsx` — enrichedToGardenPlant extraction
- `frontend/app/(garden)/plant-detail.tsx` — enrichedToGardenPlant extraction, plantId param
- `frontend/services/garden.ts` — enrichedToGardenPlant, removed dead code
- `frontend/app/_layout.tsx` — removed prevVisibleIndex ref
- `frontend/components/pages/home/statusHeader/StatusHeader.tsx` — empty guards, useEffect import
- `frontend/components/pages/garden/plantSheet/PlantSheet.tsx` — isVisible dep, plantId param
- `frontend/components/pages/garden/plantDetail/GraphModal.tsx` — removed humidity_pct

### TypeScript
- Frontend: zero errors
- Backend: zero errors

## 02/06/2026: Responsive Scaling — Tablet Support

### Plan: Responsive Scaling
We analyzed the frontend and identified that every layout value is hardcoded (Styling.ts fonts/spacing/padding, garden.ts CELL=80, tabConfig.ts, inline styles in ~25 components, and 9 components using static `Dimensions.get()`). The solution: create a single `scale.ts` utility with `scaled(n) = Math.round(n * Math.min(screenWidth / 443, 1.35))` based on the Galaxy A55 5G reference (443dp), wrap all constants, convert 9 components to `useWindowDimensions()`, and apply `scaled()` to inline values.

### Executed: Responsive Scaling (Constants Batch)
- Created `frontend/constants/scale.ts` with `scaled()` utility, scaled cap at 1.35×.
- Wrapped all numeric values in `Styling.ts` (FontSize, Spacing, Padding, BorderRadius, IconSize).
- Wrapped `garden.ts` CELL, `tabConfig.ts` BAR_HEIGHT/BAR_MARGIN/TAB_WIDTH/TAB_GAP/CORNER_RADIUS/SCOOP_RADIUS/SCOOP_DEPTH.
- These 3 constants cascade to ~40 component consumers automatically.

### Executed: Responsive Scaling (Dimensions.get → useWindowDimensions)
Converted 9 components from static `Dimensions.get()` to reactive `useWindowDimensions()`:
- **CardContainer.tsx** — replaced `Dimensions.get("window").width` with `useWindowDimensions()`, applied `scaled(40)` to padding.
- **WaveBackground.tsx** — replaced `Dimensions.get("window")`, computed `leftOffset` inside component with `scaled(75)`.
- **MonthlyCandidates.tsx** — moved `CARD_WIDTH` computation inside component using `useWindowDimensions()`.
- **GraphModal.tsx** — moved `CHART_W` inside component, updated `toX(i, total, chartW)` signature, wrapped all StyleSheet values with `scaled()`.
- **EditActionSheet.tsx** — replaced `Dimensions.get("window")` with `useWindowDimensions()`, wrapped `insets.bottom + scaled(30)`.
- **PlantSheet.tsx** — replaced `Dimensions.get("window")` with `useWindowDimensions()`, moved `maxHeight` inline.
- **OnboardingSlide.tsx** — moved `SCREEN_WIDTH/HEIGHT` inside component, converted all position constants to `scaled()`.
- **OnboardingCarousel.tsx** — moved from static `Dimensions.get()` to `useWindowDimensions()`, wrapped `insets.bottom + scaled(20)`.
- **StatusHeader.tsx** — full refactor: moved SCREEN_WIDTH/WAVE_W/WAVE_H/CONTENT_HEIGHT inside component via `useWindowDimensions()`, inlined WaveShape and CenterContent as inner components, used `scaled()` for all layout values (260, 270, 230, 180, etc.), computed `svgWrapper.left` dynamically as `-SCREEN_WIDTH + scaled(60)`.

### Executed: Responsive Scaling (Bulk Inline Values)
- Bulk PowerShell script applied `scaled()` to ~30 common hardcoded patterns across 34 component files: close buttons (32×32/br16), drag handles (40×4/br5), sheet corners (br30), paddingHorizontal (25/40/24/50), legend dots/bars, gaps, header heights (260), explore width (300), minHeights (40/56), lineHeights (20/22/28), SVG dimensions (120/150), badge positions, notification images (64×64), history items (36×36), time slot buttons (56×36), bar track heights (8/10), Spacer space values (230/175/170/150/120/110/8), WaveBackground offsets, ExploreHeader wave positioning, etc.
- **TabBarButton.tsx** — `translateY` outputRange `[0, scaled(-40)]`, iconWrapper/focusedCircle 60→`scaled(60)`, borderRadius 30→`scaled(30)`.
- **RegisterFlow.tsx** — vegImage 120×120→`scaled(120)`.
- **HistoryView.tsx** — historyTime `width: 50`→`scaled(50)`.

### Fixed: Broken Import Paths from Bulk Script
The bulk PowerShell script added `import { scaled }` to ~40 files but used incorrect relative paths. Fixed 32 files with wrong import paths (e.g. `../constants/scale` → `../../constants/scale`, `../../../constants/scale` → `../../../../constants/scale`, etc.). Also removed 3 duplicate `scaled` imports (RegisterFlow, EditActionSheet, GraphModal), fixed `CARD_WIDTH` in MonthlyCandidates (moved to inline), moved `maxHeight: SCREEN_HEIGHT * 0.75` inline in PlantSheet, and fixed 5 `paddingbottom` → `paddingBottom` typos in account views.
