...
const ResultsView = ({ results, onRestart }: Props) => (
  <StyledView>
    <View style={styles.header}>
      ...
      <StyledText type="head1" style={styles.headerTitle}>Resultaten</StyledText>
    </View>
    <Spacer space={Styling.Spacing.reg} />
    <StyledText type="paragh" style={styles.resultCount}>
      {results.length > 0
        ? `We vonden ${results.length} geschikte ${results.length === 1 ? "plant" : "planten"} voor jou!`
        : "Geen resultaten gevonden voor jouw antwoorden."}
    </StyledText>
    <Spacer space={Styling.Spacing.reg} />
    {results.length > 0 ? (
      <CardContainer
        data={results}
        onItemPress={(id) => router.push(`/(explore)/vegetable-info?id=${id}`)}
    ...
    <TouchableOpacity style={styles.restartBtn} onPress={onRestart}>
      <StyledText type="paragh" style={{ color: Styling.Colors.green }}>Opnieuw beginnen</StyledText>
    </TouchableOpacity>
